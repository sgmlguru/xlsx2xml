...
<xsl:stylesheet
    ...
    <xsl:variable name="relationships" select="/sml:workbook/sml:Relationships"/>
        ...
        <xsl:choose>
            <xsl:when test="sml:hyperlinks">
                <xsl:copy copy-namespaces="no">
                    <xsl:copy-of select="@*"/>
                    <xsl:apply-templates select="node()" mode="XLSX-UTIL_HYPERLINKS">
                        <xsl:with-param name="hyperlinks" select="sml:hyperlinks"/>
                    </xsl:apply-templates>
                </xsl:copy>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="sml:c[@t='inlineStr']" mode="XLSX-UTIL_HYPERLINKS">
        ...
        <xsl:variable name="link" select="$relationships//sml:Relationship[@Id=$rid]/@Target"/>
        <xsl:copy copy-namespaces="no">
            <xsl:copy-of select="@*"/>
