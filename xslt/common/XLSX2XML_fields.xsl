...
<xsl:stylesheet
    ...
    <xsl:template match="fc:course" mode="XLSX2XML_FIELDS">
        <xsl:variable name="fields">
            <fields>
                <xsl:apply-templates select="preceding-sibling::providermap/item[matches(@target,'^(course-field|provider-field|course-uniqueIdentifier|categories-category|course-name|duration-days|duration-weeks|duration-years|event-pace|event-price|course-link|qualification-type)$')]" mode="XLSX2XML_FIELDS"/>
            </fields>
        </xsl:variable>
