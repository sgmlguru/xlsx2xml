...
<xsl:stylesheet
    ...
    <xsl:template match="sml:sheets | sml:worksheet | sml:sheetData">
        <xsl:apply-templates select="*"/>
    </xsl:template>
    
    
    <!-- Get first row -->
    <xsl:template match="sml:row[1]">
    ...
    <!-- Delete rows 2 and onwards -->
    <xsl:template match="sml:row[position() != 1]"/>
